...
export class BlogStyleTwoComponent implements OnInit {
  ...
  constructor( private _AdminGuardGuard:AdminGuardGuard , public formBuilder:FormBuilder,private _route:Router,
  ...
  login(){
    ...
    if(response.data.isAuthenticated == true){
      if(response.data.roles.includes("Admin")){
        this._route.navigate(['BlogDetailsStyleThreeComponent'])
        // this._AdminGuardGuard.testAuth=response.data.isAuthenticated;
        this._AdminGuardGuard.testAuth=true; 
        // alert ("hello admin");
      }
      else{
